...
int Input_Charge_Density(int MD_iter, double *extpln_coes)
{
  ...
  double tmpmax;              /* MAE local spin rotation - sjkang */
  double mx, my, mz;          /* MAE local spin rotation - sjkang */
  double S_coordinate[3];     /* MAE local spin rotation - sjkang */
  ...
  if ( i_vec[1]==Ngrid1 && i_vec[2]==Ngrid2 && i_vec[3]==Ngrid3 ){
    ...
    if (Use_of_Collinear_Restart==1){
      ...
      ******************************************************************************/
      ...
      if (Restart_Spin_Mixing_Switch == 0){
	...
	Density_Grid_B[3][BN] = Im12;
      ...
      /* >> MAE local spin rotation - sjkang */
      printf("Restarg spin switch: %i\n", Restart_Spin_Mixing_Switch);
      if ( (Restart_Spin_Mixing_Switch == 2) || (Restart_Spin_Mixing_Switch == 3)){
        if (myid==0){
          if(Restart_Spin_Mixing_Switch == 2) printf("Restart Spin Mixing : Average\n"); fflush(stdout);
          if(Restart_Spin_Mixing_Switch == 3) printf("Restart Spin Mixing : Oneside\n"); fflush(stdout);
        }
        for (BN=0; BN<My_NumGridB_AB; BN++){
          ratiosum = 0.0;
          t = 0.0;
          ...
          for (m=1; m<=atomnum; m++){
            ...
            ratiosum += Density_Grid_B_Atom[m][0][BN] + Density_Grid_B_Atom[m][1][BN];

            t = Restart_Spin_Angles[m][0]/180.0*PI*0.5; 
            p = Restart_Spin_Angles[m][1]/180.0*PI*0.5; 
            mx += ratio[m] * sin(t) * cos(p); 
            my += ratio[m] * sin(t) * sin(p); 
            mz += ratio[m] * cos(t);
          }
          if (ratiosum > 1.E-18){
            if (Restart_Spin_Mixing_Switch == 2){
              xyz2spherical(mx, my, mz, 0.0, 0.0, 0.0, S_coordinate); 
              t = S_coordinate[1];
              p = S_coordinate[2];
            }
            else if (Restart_Spin_Mixing_Switch == 3){
              tmpmax = ratio[1];
              t = Restart_Spin_Angles[1][0]/180.0*PI*0.5; 
              p = Restart_Spin_Angles[1][1]/180.0*PI*0.5; 
              for (m=1; m<=atomnum; m++){
                if (ratio[m] >= tmpmax){
                  t = Restart_Spin_Angles[m][0]/180.0*PI*0.5; 
                  p = Restart_Spin_Angles[m][1]/180.0*PI*0.5; 
      ...
      else if (Restart_Spin_Mixing_Switch == 1){
        if (myid==0) printf("Restart Spin Mixing : Atom \n"); fflush(stdout);     
        for (BN=0; BN<My_NumGridB_AB; BN++){
